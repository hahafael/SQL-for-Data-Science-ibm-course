...
--Query A1: Enter a failing (i.e. which gives an error) to retrieve all employees whose salary
-- is greater than the average salary

select * from employees where salary > AVG(salary);

--Query A2: Enter a working query using a sub-select to retrieve all employees 
...
--Query A4: Enter a Column Expression that retrieves all employees records and average salary in
-- every row
...
--Query B1: Retrieve only the EMPLOYEES records that correspond to departments 
